...
def build_turret_wall_pair_game(
    game_state: GameState,
    ...
    can_build_turret = not game_state.contains_stationary_unit(location=turret_location)

    if total_cost < game_state.get_resource(game_state.SP) and can_build_turret:
        game_state.attempt_spawn(game_state.TURRET, turret_location)  # Build the turret
        built += 1
...
def build_turret_wall_pair_map(
    game_map: GameMap,
    turret_location: (int, int) or [[int]],
    turret_id: int,
    wall_id: int,
